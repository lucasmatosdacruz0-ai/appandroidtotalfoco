...
const FocusSelector: React.FC<FocusSelectorProps> = ({ onSelect, currentFocus }) => {
  return (
    <div className="text-center">
      <h2 className="text-2xl sm:text-3xl font-bold text-gray-100">Qual o foco do seu treino?</h2>
      <p className="text-gray-400 mt-2 mb-8">Vamos direcionar os exercícios para o seu objetivo de hoje.</p>
            ...
            >
              {recommended && (
                <div className="absolute top-0 right-0 transform translate-x-1/3 -translate-y-1/3 z-10">
                    <span className="flex items-center px-2.5 py-1 bg-green-500 text-white text-xs font-bold rounded-full shadow-lg ring-2 ring-gray-800">
                      <StarIcon className="w-3.5 h-3.5 mr-1" />
                      Recomendado
                    </span>
                </div>
              )}
              <div className="text-blue-500 group-hover:text-blue-400 transition-colors h-12">{icon}</div>
              <h3 className="text-xl font-bold text-gray-200 mt-2">{focus}</h3>
              <p className="text-gray-400 mt-2 text-sm flex-grow min-h-[50px]">{description}</p>
            </button>
          )
        })}
      </div>

...
const ComparisonCard: React.FC<{ comparison: ExerciseComparison }> = ({ comparison }) => {
    return (
        <div className="bg-gray-700/50 p-4 rounded-lg border border-gray-600">
            <h4 className="font-bold text-lg text-gray-100 flex items-center">
                {comparison.name}
                {comparison.isNewPR && <span className="ml-2 text-xl animate-bounce">🚀</span>}
            </h4>
            <div className="mt-2 grid grid-cols-2 gap-4 text-sm">
                <div>
                    <p className="font-semibold text-gray-300">Neste Treino:</p>
                    <p className="text-gray-200"><span className="font-bold">{comparison.current.maxWeight} kg</span> (max)</p>
                </div>
                <div>
                     <p className="font-semibold text-gray-300">Última Vez:</p>
                     {comparison.last ? (
                        <p className={`text-gray-200 ${comparison.isNewPR ? 'text-green-400 font-bold' : ''}`}>
                            <span className="font-bold">{comparison.last.maxWeight} kg</span> (max)
                        </p>
                     ) : (
                        <p className="text-gray-400 italic">Primeira vez</p>
                     )}
                </div>
            </div>
        </div>
...
const WorkoutSummary: React.FC<WorkoutSummaryProps> = ({ summary, onDone, postWorkoutInfo }) => {
    return (
        <div className="bg-gray-800 rounded-2xl shadow-xl p-6 sm:p-8 animate-fade-in max-w-2xl mx-auto">
                ...
                {postWorkoutInfo.didLevelUp && (
                    <div className="mb-4 text-center animate-fade-in">
                        <h2 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-500">
                         ...
                         <p className="text-lg text-gray-300">Você alcançou o Nível {postWorkoutInfo.newLevel}!</p>
                    </div>
                )}
                <h2 className="text-3xl sm:text-4xl font-extrabold text-white">Resumo do Treino</h2>
                <p className="mt-2 text-lg text-gray-300">{summary.title}</p>
                <p className="font-bold text-green-500 mt-2">Parabéns por concluir seu treino!</p>
            </div>

            <div className="my-8 grid grid-cols-2 sm:grid-cols-3 gap-4 border-y border-gray-700 py-6">
            ...
            <div>
                <h3 className="text-xl font-bold text-white mb-4">Seu Desempenho</h3>

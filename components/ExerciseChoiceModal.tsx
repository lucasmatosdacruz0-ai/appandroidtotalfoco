    ...
    >
      ...
      >
        ...
        <div className="text-center">
            <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2 pr-8">Escolha uma Alternativa</h2>
            <p className="text-gray-400">Alternativas para: <span className="font-semibold text-gray-300">{exerciseToReplaceName}</span></p>
        </div>

        <div className="mt-6 space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
          {choices.length > 0 ? choices.map((choice) => (
            <div key={choice.id} className="bg-gray-700/50 p-4 rounded-lg border border-gray-600">
                <div className="flex items-center justify-between">
                    <h3 className="text-lg font-bold text-gray-100">{choice.name}</h3>
                    <div className="flex items-center gap-2 text-sm font-semibold shrink-0">
                        <span className="bg-blue-900/70 text-blue-200 px-2 py-1 rounded-full">{choice.sets} séries</span>
                        <span className="bg-green-900/70 text-green-200 px-2 py-1 rounded-full">{choice.reps} reps</span>
                    </div>
                </div>
                ...
                <div className="mt-3 flex items-center justify-end gap-3 border-t border-gray-600 pt-3">
                   <button 
            ...
            <p className="text-center text-gray-400 py-8">Nenhuma alternativa encontrada.</p>
          )}
        </div>
      </div>
    </div>

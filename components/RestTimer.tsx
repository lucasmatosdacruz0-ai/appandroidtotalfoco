  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-gray-800 rounded-2xl shadow-xl w-full max-w-sm p-6 relative text-center" onClick={e => e.stopPropagation()}>
          ...
          <h2 className="text-2xl font-bold text-white mb-6">Cronômetro de Descanso</h2>
          ...
          {isFinished ? (
            <div className="flex flex-col items-center justify-center animate-fade-in py-8">
              ...
              <h3 className="text-2xl font-bold text-white">Descanso Concluído!</h3>
              <p className="text-gray-400 mt-2 mb-8">Prepare-se para a próxima.</p>
              <button
                  onClick={onNextSet}
            ...
            <>
              ...
              {isRunning ? (
                <div className="flex justify-center items-center gap-4 mt-8">
                    <button onClick={handleToggle} className="p-4 rounded-full bg-gray-600 hover:bg-gray-500">
                        {isActive ? <PauseIcon className="w-8 h-8 text-gray-100" /> : <PlayIcon className="w-8 h-8 text-gray-100" />}
                    </button>
                    <button onClick={resetTimer} className="p-4 rounded-full bg-gray-600 hover:bg-gray-500">
                        <RefreshCwIcon className="w-8 h-8 text-gray-100" />
                    </button>
                </div>
              ) : (
                <div className="mt-4 animate-fade-in">
                  <h3 className="text-sm font-semibold text-gray-400 mb-3">Opções Rápidas</h3>
                  <div className="grid grid-cols-3 gap-2 mb-6">
                    {presetTimes.map(preset => (
                      <button 
                        key={preset.label}
                        onClick={() => startTimer(preset.seconds)}
                        className="bg-blue-500/20 text-blue-300 font-bold py-2 rounded-lg hover:bg-blue-500/30 transition-colors"
                      >
                        {preset.label}
                      </button>
                    ))}
                  ...
                  <div className="border-t border-gray-700 pt-4">
                    <h3 className="text-sm font-semibold text-gray-400 mb-3">Ou insira um tempo:</h3>
                    <div className="flex items-center justify-center gap-2 mb-4">
                        <input type="tel" value={minutes} onChange={handleMinuteChange} className="w-20 p-2 text-2xl font-bold bg-gray-700 text-center rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        <span className="text-2xl font-bold text-gray-400">:</span>
                        <input type="tel" value={seconds} onChange={handleSecondChange} className="w-20 p-2 text-2xl font-bold bg-gray-700 text-center rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
